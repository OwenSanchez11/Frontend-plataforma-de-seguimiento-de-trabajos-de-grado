<script>
	...
	// Detecta si estamos dentro del modo administrador
	let esAdmin = $derived($page.url.pathname.startsWith('/admin'));
...
<header class="bg-white border-bottom sticky-top shadow-sm">
	...
	<div class="container-fluid px-3 px-md-4 py-2 d-flex align-items-center justify-content-between">
		...
		<a
			class="navbar-brand d-flex align-items-center gap-2 fw-bold text-primary mb-0"
			href="/"
			onclick={cerrarMenu}
		>
			<div
				class="bg-primary text-white rounded-3 p-1.5 d-flex align-items-center justify-content-center"
			...
			<span class="lh-sm">
				<span class="d-block h6 fw-bold text-dark mb-0">
					GradosTrack
				</span>
			</span>
		</a>
		...
		<div class="d-flex align-items-center gap-2">
			...
			>
				<div
					class="bg-primary text-white rounded-circle d-flex align-items-center justify-content-center fw-bold extra-small flex-shrink-0"
				...
				<div class="lh-1 min-w-0 d-none d-sm-block">
					<span class="fw-bold text-dark d-block extra-small text-truncate">
						{usuario.nombre}
					</span>
	...
	<div
		class="collapse navbar-collapse-custom border-top {menuAbierto ? 'show' : ''}"
		id="navbarContenido"
	>
		<div
			class="container-fluid px-3 px-md-4 py-2 d-flex flex-column flex-xl-row align-items-stretch align-items-xl-center justify-content-between gap-2"
		>
			...
			<ul class="nav flex-column flex-xl-row gap-1 mb-0 flex-grow-1">
				...
				{#each menuPrincipal as item}
					<li class="nav-item">
						<a
							class="nav-link px-2.5 py-2 rounded-3 extra-small fw-semibold d-flex align-items-center gap-2 transition-all {$page.url.pathname === item.href
				...
				<!-- EVALUACIONES SOLO SE MUESTRA FUERA DEL MODO ADMIN -->
				...
				{#if !esAdmin}
					...
					<li class="nav-item d-xl-none mt-2 pt-2 border-top">
						<span class="extra-small text-muted fw-bold text-uppercase px-2 d-block mb-1">
